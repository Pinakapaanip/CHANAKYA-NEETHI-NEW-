...
class Enemy(BaseEntity):
    def __init__(self, x, y, enemy_type="soldier"):
        ...
        self.walk_sprites = self._load_walk_sprites()
    ...
    def _load_walk_sprites(self):
        if self.enemy_type == "soldier":
            ...
            candidates = [
                "enemies/archer_walk_01.png",
                "enemies/archer_walk_02.png",
            ]
        frames = [load_first_image([path], size=(self.rect.width, self.rect.height)) for path in candidates]
    ...
    def update_ai(self, player_rect, dt):
        ...
        if moved and self.walk_sprites:
            self.walk_anim_timer += dt
            if self.walk_anim_timer >= 0.12:
                self.walk_anim_timer = 0
                self.walk_frame_index = (self.walk_frame_index + 1) % len(self.walk_sprites)
    ...
    def draw(self, surface):
        ...
        elif self.hostile and self.walk_sprites:
            sprite = self.walk_sprites[self.walk_frame_index]
        else:
